...
class SpanningForest (object):
  ...
  def _handle_openflow_PortStatus (self, e):
    ...
    if l is not None:
      ...
      if l.liveness != prev_liveness:
        self._compute()
        self.timeCount += 1
        self.Time += time.process_time() - timeNow
        log.debug("Average time to calculate for path failures:" + str(self.Time/self.timeCount))
  def _handle_openflow_discovery_LinkEvent (self, e):
    ...
    if link.liveness != prev_liveness:
      self._compute()
      self.everyTimeCount += 1
      self.everyTime += time.process_time() - timeNow
      log.debug("Average time to calculate for all cases:" + str(self.everyTime/self.everyTimeCount))
